...
function PLUGIN:Call( ply, args )
	if ( ply:EV_HasPrivilege( "Cleanup" ) ) then
		if ( #args == 0 ) then
			...
			game.CleanUpMap(true, evolve.ignore_classes)
		else
...
hook.Add("InitPostEntity", "EvolveInitPostEntityCleanUpFix", function()
	for _, ent in ipairs( ents.GetAll() ) do
		local class = ent:GetClass()
		if !table.HasValue(evolve.ignore_classes, class) then table.insert(evolve.ignore_classes, class) end
	end
end)